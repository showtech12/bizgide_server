...
const {UserSchema, UserSchemaEdit} = require('../../validate/validateUserInput');
...
router.post("/upload", (req, res) => {
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      res.status(500).send({ message: 'Error writing file to disk', error: writeError.message });
    }
...
router.get("/api/v1/users",verifyAdmin, pagination, AuthRole('ADMIN'),
  async (req, res) => {
...
router.post("/api/v1/user", verifyAdmin, AuthRole("ADMIN"), async (req, res) => {
  try {
...
router.post("/api/v1/userupdate",verifyAdmin,AuthRole('ADMIN'), async (req, res) => {
...
router.post("/api/v1/deleteuser",verifyAdmin, AuthRole('ADMIN'), idNumControlPOST,  async (req, res, next) => {
...
router.get("/api/v1/verify_token", verifyAdmin, AuthRole('ADMIN','CASHIER','USER'), async (req, res, next) => {
