...
router.post("/api/v1/updtpass", verifyAdmin, async (req, res, next) => {
  ...
  let OldPass = req.userDtl[0].pass_Word;
  ...
  const schema = Joi.object({
    ...
    txtNewPassW: Joi.string().min(8).required().messages({
...
router.post("/api/v1/custsettings", verifyAdmin, async (req, res, next) => {
  ...
  sequelize
    .query(`UPDATE persons SET isActive1 = :isActive1 WHERE id= :id`, {
...
router.post("/api/v1/profilepix", uploadMiddleware, (req, res) => {
  ...
  sequelize
    .query(`UPDATE persons SET passport_path = :passport_path WHERE id= :id`, {
...
router.post("/api/v1/person", async (req, res, next) => {
  ...
  if (error) {
    ...
    const PrsRecP = await cPersons.getBySingleCol(
      "phone_num",
      req.body.txtPhone_no,
    );
    if (PrsRecP) {
      return res.status(200).json({
        code: 200,
    ...
    SndMail.getSendMail(
      ...
      MyMail,
      "Verify Account",
      req.body.txtSurname.toUpperCase(),
    );
