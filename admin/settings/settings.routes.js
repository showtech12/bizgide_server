    ...
    await getTransact.checkoutWithRetry(async () => {
      try {
        ...
        const settingsSchema = Joi.object({
          expiringAlertPercent: Joi.number()
            .min(0)
            .max(100)
            .required()
            .messages({
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      sequelize
        .query(
          `  SELECT expiring_perct, stock_out_perct, profit_margin_perct FROM tblsettings WHERE clt_id = '${client_id}'`,
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      sequelize
        .query(
          `SELECT id,rolename, permission FROM tblrole WHERE clt_id='${client_id}'`,
          {
            type: sequelize.QueryTypes.SELECT,
          },
        )
